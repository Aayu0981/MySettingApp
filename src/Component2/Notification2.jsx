...
//     } catch (error) {
//       console.error('Error toggling smarthome mode', error.message);
//     }
//   };
...
function Notification2() {
  ...
  const toggleSmarthome = () => {
    setIsSmarthomeOn(!isSmarthomeOn);
    // Update the setting in the database when the user toggles it
    postSmarthomeToServer(!isSmarthomeOn);
  };

  const postSmarthomeToServer = async (newValue) => {
    try {
      const response = await axios.post(`${BASE_URL}/smarthometogle`, {
        tog: newValue, // Pass the boolean value directly
      });
      console.log(response.data);
    } catch (error) {
      console.error('Error toggling smarthome mode', error.message);
  ...
  return (
    <div style={{ width: 350 }}>
      ...
      <p className="heading" style={{ marginLeft: 25 }}>
        Additional cards
      </p>

      <div className="Modes" style={{ marginLeft: 30 }}>
        <p className="Darktext1" style={{ marginLeft: 0 }}>
          Smart home
        </p>
        <label className="switch">
          <input type="checkbox" onChange={toggleSmarthome} checked={isSmarthomeOn} />
          <span className="slider round"></span>
        </label>
      </div>
    </div>
